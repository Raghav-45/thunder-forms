...
interface InteractionPaletteItem {
  id: string
  fieldType: avaliableFieldsType
}

function StaticPaletteV5() {
  return (
    <div className="grid grid-cols-2 gap-2 md:flex md:flex-col items-start flex-wrap md:flex-nowrap gap-y-2 overflow-y-auto w-full">
      <div
        className="flex h-8 w-full items-center rounded-lg border border-input bg-neutral-900 px-2 text-sm md:pl-3"
      >
        <div className="overflow-hidden truncate text-[0.625rem] md:text-xs">
          Add Section
        </div>
      ...
      {(AVAILABLE_FIELDS as avaliableFieldsType[]).map((fieldType) => (
        <div
          key={fieldType}
          className="flex h-8 w-full items-center rounded-lg border border-input bg-neutral-900 px-2 text-sm md:pl-3"
        >
      ...
      {comingSoonElements.map((fieldType) => (
        <div
          key={fieldType}
          className="flex h-8 w-full items-center rounded-lg border border-input bg-neutral-900 px-2 text-sm md:pl-3 opacity-70"
        >
...
interface InteractionPaletteV5Props {
  items: InteractionPaletteItem[]
  onAddSection: () => void
  onAddField: (fieldType: avaliableFieldsType) => void
}

const InteractionPaletteV5 = memo(function InteractionPaletteV5({
  items,
  onAddSection,
  onAddField,
}: InteractionPaletteV5Props) {
  return (
    <div className="grid grid-cols-2 gap-2 md:flex md:flex-col items-start flex-wrap md:flex-nowrap gap-y-2 overflow-y-auto w-full">
      <Button
        variant="outline"
        className="rounded-lg w-full px-2 md:pl-3 bg-neutral-900! cursor-pointer opacity-0"
        size="sm"
      ...
      {comingSoonElements.map((fieldType) => (
        <Button
          key={fieldType}
          variant="outline"
          className="rounded-lg w-full px-2 md:pl-3 bg-neutral-900! cursor-not-allowed opacity-0"
          size="sm"
          disabled
          tabIndex={-1}
        >
...
interface InteractionPaletteFieldV5Props {
  item: InteractionPaletteItem
  onAdd: (fieldType: avaliableFieldsType) => void
}

const InteractionPaletteFieldV5 = memo(function InteractionPaletteFieldV5({
  item,
  onAdd,
}: InteractionPaletteFieldV5Props) {
  ...
  return (
    <button
      ...
      className={cn(
        ...
        'rounded-lg w-full px-2 md:pl-3 bg-neutral-900! cursor-grab opacity-0',
      )}
      onClick={() => onAdd(item.fieldType)}
      aria-label={`Add ${item.fieldType}`}
    >
      <PaletteFieldContent fieldType={item.fieldType} />
    </button>
  )
})

function EmptyCanvasDropZoneV5() {
...
export default function TestingV5Page() {
  return (
    ...
    >
      <TestingV5Builder params={Promise.resolve({ slug: '' })} />
    </Suspense>
  )
}

function TestingV5Builder({ params }: FormBuilderProps) {
  ...
  >(() =>
    (AVAILABLE_FIELDS as avaliableFieldsType[]).map((fieldType, index) => ({
      id: `v5-palette-${fieldType}-${index}`,
      fieldType,
    })),
  )
  const refillSequence = useRef(0)
  ...
  const refillPaletteItem = useCallback((sourceId: string) => {
    setInteractionItems((items) => {
      const sourceIndex = items.findIndex((item) => item.id === sourceId)
      const sourceItem = items[sourceIndex]
      if (!sourceItem) return items

      refillSequence.current += 1
      const nextItems = [...items]
      nextItems[sourceIndex] = {
        ...sourceItem,
        id: `v5-palette-${sourceItem.fieldType}-refill-${refillSequence.current}`,
      }
      return nextItems
    })
  }, [])
  ...
  const handleDragEnd = useCallback<DragDropEventHandlers['onDragEnd']>(
    (event) => {
      ...
      if (event.canceled) {
        ...
        if (source?.type === PALETTE_FIELD_TYPE) {
          refillPaletteItem(String(source.id))
        }
        return
      }

      if (source?.type !== PALETTE_FIELD_TYPE) return
    ...
    [insertPaletteField, refillPaletteItem],
  )
    ...
    <div className="flex bg-background h-screen text-foreground">
      ...
      >
        ...
        <div className="flex flex-row justify-between">
          <h2 className="mb-6 font-bold text-3xl">Testing v5 · Builder</h2>
        ...
        <Card className="hidden md:block border-0 border-l-2 rounded-none w-80 h-screen overflow-hidden">
          <CardContent className="p-4 pt-0">
            ...
            <CardDescription>
              Smooth drag layer over a static palette
            </CardDescription>
