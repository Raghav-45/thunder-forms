// components/FormBuilder/utils/validation.ts
...
export const validateField = (field: FieldConfig, value: any): string[] => {
  ...
  switch (field.type) {
    case 'text-input':
      validateTextInput(field as import('../elements').TextInputConfig, value, errors);
      break;
    case 'multi-select':
      validateMultiSelect(field as import('../elements').MultiSelectConfig, value, errors);
      break;
  }
...
const validateTextInput = (
  ...
  if (field.pattern) {
    try {
      ...
      console.warn(`Invalid regex pattern for field ${field.id}:`, field.pattern);
    }
  }
